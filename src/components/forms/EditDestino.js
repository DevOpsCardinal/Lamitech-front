...
import { traerDestino, updateDestino } from "API/destino";
  ...
  export default function EditarDestino(){
    ...
    const formik = useFormik({
        ...
        onSubmit: async (formValue) => {
          const response = await updateDestino(formValue, auth, busqueda)
      ...
      const response = await traerDestino(proceso,valor, auth)
      ...
      return (
        <tbody  >
          ...
          <Form onSubmit={formik.handleSubmit}>
            <h4 style={{ color: '#fff' }}>Actualizar Destino</h4>
