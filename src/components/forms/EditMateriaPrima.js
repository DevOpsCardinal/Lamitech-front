...
import { traerMateriaPrima, updateMateriaPrima } from "API/materiaPrima";
...
export default function EditarMateriaPrima(){
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) =>{
      const response = await updateMateriaPrima(formValue, auth, busqueda)
  ...
  const response = await traerMateriaPrima(proceso,valor, auth)
  console.log('[EditMateriaPrima]'+ response);
      ...
      <tr >
        <td>
          ...
          <Form onSubmit={formik.handleSubmit}>
            <h4 style={{ color: '#fff' }}>Registrar Materia Prima</h4>
