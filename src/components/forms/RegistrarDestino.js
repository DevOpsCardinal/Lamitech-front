...
import { createDestinoApi } from 'API/destino';
...
export default function RegistrarDestino() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (formValue) => {
      const response = await createDestinoApi(formValue, auth)
      ...
      <tr >
        <td>
          ...
          <Form onSubmit={formik.handleSubmit}>
            <h4 style={{ color: 'white' }}>Registrar Destino</h4>
            <Row style={{ alignContent: 'center', justifyContent: 'center', alignItems: 'center', marginBottom: 10 }}>
              ...
              <Col style={{ marginTop: 10 }} className="pl-1" md="5">
                <Form.Group>
                  <label style={{ color: 'white' }}>Detalles</label>
                  <Form.Control
                    name="detalles"
